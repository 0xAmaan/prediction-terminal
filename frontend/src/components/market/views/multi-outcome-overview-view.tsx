...
import type { PredictionMarket, MarketOption, Trade } from "@/lib/types";

// Components
import { MultiOutcomeChart } from "@/components/market/multi-outcome-chart";
import { OutcomeSummaryGrid } from "@/components/market/overview/outcome-summary-grid";
...
const StatCard = ({
  ...
  >
    ...
    {subValue && (
      <div className="text-xs mt-1" style={{ color: fey.grey500 }}>
        {subValue}
      </div>
    )}
  </motion.div>
...
export interface MultiOutcomeOverviewViewProps {
...
export const MultiOutcomeOverviewView = ({
  market,
  options,
  trades = [],
  onOutcomeClick,
}: MultiOutcomeOverviewViewProps) => {
  // Get leading outcome
  const sortedOptions = [...options].sort(
    (a, b) => parseFloat(b.yes_price) - parseFloat(a.yes_price)
  );
  const leadingOutcome = sortedOptions[0];
  const leadingPrice = leadingOutcome
    ? `${(parseFloat(leadingOutcome.yes_price) * 100).toFixed(0)}%`
    : "—";
    ...
    <div className="flex-1 overflow-auto">
      ...
      <div className="px-6 lg:px-8 py-8 pb-24">
        ...
        >
          ...
          {/* 2. Multi-Outcome Price Chart - All outcomes over time */}
          ...
          {/* 3. Outcome Summary Grid - Top outcomes with prices */}
          <motion.div variants={staggerItem}>
            <OutcomeSummaryGrid
              options={options}
              maxDisplay={6}
          ...
          {/* 4. Key Stats Grid - Horizontal mini-cards (Fey style) */}
          <motion.div
          ...
          >
            <StatCard
              label="Leading"
              value={leadingOutcome?.name ?? "—"}
              valueColor={fey.teal}
              subValue={leadingPrice}
            />
            <StatCard
              label="Outcomes"
              value={options.length.toString()}
              subValue="Total options"
            />
            <StatCard
              label="24h Volume"
          ...
          {/* 5. Historical Analysis - Compact horizontal stats */}
          <motion.div variants={staggerItem}>
            <HistoricalAnalysisCard
              priceHistory={[]} // TODO: Could aggregate from all outcomes
              volume24h={market.volume}
              tradeCount={trades.length}
            />
          </motion.div>

          {/* 6. Resolution Strategy - Market rules and resolution info */}
          <motion.div variants={staggerItem}>
...
export default MultiOutcomeOverviewView;
